...
//             }
//             auctionDocData.auctions.forEach(auctionData => {
//                 if (auctionData.StartBid >= minBid && auctionData.StartBid <= maxBid) {
//                     auctionsArray.push({...auctionData, id: doc.id}); // Include document ID
//                 }
//             });
//         });
...
//         // Sort auctions by StartBid
//         console.log(sortOption)
//         if (sortOption == 'lowest') {
//             console.log('lowest>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
//             auctionsArray.sort((a, b) => parseInt(a.StartBid) - parseInt(b.StartBid));
//         } else if (sortOption == 'highest') {
//             console.log('highest>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')

//             auctionsArray.sort((a, b) => parseInt(b.StartBid) - parseInt(a.StartBid));
//         }
//         console.log('auctionsArray after sorting:', auctionsArray);
